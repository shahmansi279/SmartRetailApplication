...
public class EventDetailActivity extends FragmentActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		name = (TextView) findViewById(R.id.event_title);
		desc = (TextView) findViewById(R.id.event_desc);
		addr = (TextView) findViewById(R.id.event_addr);
		timing = (TextView) findViewById(R.id.event_timing);

		Intent t = getIntent();

		this.username = t.getStringExtra("username");
		this.password = t.getStringExtra("password");
		this.eventName = t.getStringExtra("eTitle");
		String eventAddr = t.getStringExtra("eAddr");
		String eventDesc = t.getStringExtra("eDesc");
		String eventTiming ="Open : "+ t.getStringExtra("eTiming");
		String url = t.getStringExtra("eIcon");
		name.setText(eventName);
		desc.setText(eventDesc);
		addr.setText(eventAddr);
		timing.setText(eventTiming);
	...
	public void sendEmail(View v) {
		...
		t.putExtra("eventName", this.eventName);
	...
	public void plotMap(String urls) throws UnsupportedEncodingException {
		...
		// Start downloading the geocoding events
	...
	class DownloadTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPostExecute(String result) {
			...
			// Start parsing the events in JSON format
	...
	class ParserTask extends
		...
		protected void onPostExecute(List<HashMap<String, String>> list) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				// Getting a event from the events list
				HashMap<String, String> hmPlace = list.get(i);

				// Getting latitude of the event
				double lat = Double.parseDouble(hmPlace.get("lat"));

				// Getting longitude of the event
